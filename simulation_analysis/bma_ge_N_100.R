...
load("./simulations/data/data_ge_N_100.RData")
...
bma_m1_100_02 <- vector(mode = 'list',
                        length = M)

bma_m1_100_05 <- vector(mode = 'list',
                        length = M)

bma_m1_100_1 <- vector(mode = 'list',
                       length = M)

# Create a list to hold the adjacency matrices created from the bma output.
ppm_m1_100_02 <- vector(mode = 'list',
                        length = M)

ppm_m1_100_05 <- vector(mode = 'list',
                        length = M)

ppm_m1_100_1 <- vector(mode = 'list',
                       length = M)

# Create lists to for the precision and power.
bma_prerec_m1_100_02 <- vector(mode = 'list',
                               length = M)

bma_prerec_m1_100_05 <- vector(mode = 'list',
                               length = M)

bma_prerec_m1_100_1 <- vector(mode = 'list',
                              length = M)

# Create a list to hold the runtime for each data set.
times_m1_100_02 <- vector(mode = 'list',
                          length = M)

times_m1_100_05 <- vector(mode = 'list',
                          length = M)

times_m1_100_1 <- vector(mode = 'list',
                         length = M)

# Topology M2 --------------------------

# Create a list to hold the output from each node in m1.
bma_m2_100_02 <- vector(mode = 'list',
                        length = M)

bma_m2_100_05 <- vector(mode = 'list',
                        length = M)

bma_m2_100_1 <- vector(mode = 'list',
                       length = M)

# Create a list to hold the adjacency matrices created from the bma output.
ppm_m2_100_02 <- vector(mode = 'list',
                        length = M)

ppm_m2_100_05 <- vector(mode = 'list',
                        length = M)

ppm_m2_100_1 <- vector(mode = 'list',
                       length = M)

# Create lists to for the precision and power.
bma_prerec_m2_100_02 <- vector(mode = 'list',
                               length = M)

bma_prerec_m2_100_05 <- vector(mode = 'list',
                               length = M)

bma_prerec_m2_100_1 <- vector(mode = 'list',
                              length = M)

# Create a list to hold the runtime for each data set.
times_m2_100_02 <- vector(mode = 'list',
                          length = M)

times_m2_100_05 <- vector(mode = 'list',
                          length = M)

times_m2_100_1 <- vector(mode = 'list',
                         length = M)
...
for (e in 1:M) {
  ...
  bma_m1_100_02[[e]] <- vector(mode = 'list',
                               length = dim(data_m1_100_02[[1]])[2])

  bma_m1_100_05[[e]] <- vector(mode = 'list',
                               length = dim(data_m1_100_05[[1]])[2])

  bma_m1_100_1[[e]] <- vector(mode = 'list',
                              length = dim(data_m1_100_1[[1]])[2])

  begin_time <- Sys.time()
  # Apply ScanBMA to each node in the network.
  for (v in 1:dim(data_m1_100_02[[1]])[2]) {

    bma_m1_100_02[[e]][[v]] <- ScanBMA(x = data_m1_100_02[[e]][, -v],
                                       y = data_m1_100_02[[e]][, v],
                                       prior.prob = 0.1,
                                       control = control)
  ...
  times_m1_100_02[[e]] <- end_time - begin_time

  begin_time <- Sys.time()
  # Apply ScanBMA to each node in the network.
  for (v in 1:dim(data_m1_100_05[[1]])[2]) {

    bma_m1_100_05[[e]][[v]] <- ScanBMA(x = data_m1_100_05[[e]][, -v],
                                       y = data_m1_100_05[[e]][, v],
                                       prior.prob = 0.1,
                                       control = control)
  ...
  # Calculate the runtime for each graph.
  times_m1_100_05[[e]] <- end_time - begin_time

  begin_time <- Sys.time()
  # Apply ScanBMA to each node in the network.
  for (v in 1:dim(data_m1_100_1[[1]])[2]) {

    bma_m1_100_1[[e]][[v]] <- ScanBMA(x = data_m1_100_1[[e]][, -v],
                                      y = data_m1_100_1[[e]][, v],
                                      prior.prob = 0.1,
                                      control = control)
  ...
  # Calculate the runtime for each graph.
  times_m1_100_1[[e]] <- end_time - begin_time

  # Topology M2 --------------------------

  # ScanBMA must be applied to each gene individually.
  # Create a sublist to hold the output for each node.
  bma_m2_100_02[[e]] <- vector(mode = 'list',
                               length = dim(data_m2_100_02[[1]])[2])

  bma_m2_100_05[[e]] <- vector(mode = 'list',
                               length = dim(data_m2_100_05[[1]])[2])

  bma_m2_100_1[[e]] <- vector(mode = 'list',
                              length = dim(data_m2_100_1[[1]])[2])

  begin_time <- Sys.time()
  # Apply ScanBMA to each node in the network.
  for (v in 1:dim(data_m2_100_02[[1]])[2]) {

    bma_m2_100_02[[e]][[v]] <- ScanBMA(x = data_m2_100_02[[e]][, -v],
                                       y = data_m2_100_02[[e]][, v],
                                       prior.prob = 0.1,
                                       control = control)
  ...
  # Calculate the runtime for each graph.
  times_m2_100_02[[e]] <- end_time - begin_time

  begin_time <- Sys.time()
  # Apply ScanBMA to each node in the network.
  for (v in 1:dim(data_m2_100_05[[1]])[2]) {

    bma_m2_100_05[[e]][[v]] <- ScanBMA(x = data_m2_100_05[[e]][, -v],
                                       y = data_m2_100_05[[e]][, v],
                                       prior.prob = 0.1,
                                       control = control)
  ...
  # Calculate the runtime for each graph.
  times_m2_100_05[[e]] <- end_time - begin_time

  begin_time <- Sys.time()
  # Apply ScanBMA to each node in the network.
  for (v in 1:dim(data_m2_100_1[[1]])[2]) {

    bma_m2_100_1[[e]][[v]] <- ScanBMA(x = data_m2_100_1[[e]][, -v],
                                      y = data_m2_100_1[[e]][, v],
                                      prior.prob = 0.1,
                                      control = control)
  ...
  # Calculate the runtime for each graph.
  times_m2_100_1[[e]] <- end_time - begin_time
...
for (e in 1:M) {
  ...
  ppm_m1_100_02[[e]] <- am_bma(bma_m1_100_02[[e]])
  ppm_m1_100_05[[e]] <- am_bma(bma_m1_100_05[[e]])
  ppm_m1_100_1[[e]] <- am_bma(bma_m1_100_1[[e]])

  # Calculate the precision and power from the adjacency matrices.
  bma_prerec_m1_100_02[[e]] <- prerec(amInferred = ppm_m1_100_02[[e]],
                                      amTrue = am_m1,
                                      cutoff = 0.5)

  bma_prerec_m1_100_05[[e]] <- prerec(amInferred = ppm_m1_100_05[[e]],
                                      amTrue = am_m1,
                                      cutoff = 0.5)

  bma_prerec_m1_100_1[[e]] <- prerec(amInferred = ppm_m1_100_1[[e]],
                                     amTrue = am_m1,
                                     cutoff = 0.5)
  ...
  # Create posterior probability adjacency matrices from bma output.
  ppm_m2_100_02[[e]] <- am_bma(bma_m2_100_02[[e]])
  ppm_m2_100_05[[e]] <- am_bma(bma_m2_100_05[[e]])
  ppm_m2_100_1[[e]] <- am_bma(bma_m2_100_1[[e]])

  # Calculate the precision and power from the adjacency matrices.
  bma_prerec_m2_100_02[[e]] <- prerec(amInferred = ppm_m2_100_02[[e]],
                                      amTrue = am_m2,
                                      cutoff = 0.5)

  bma_prerec_m2_100_05[[e]] <- prerec(amInferred = ppm_m2_100_05[[e]],
                                      amTrue = am_m2,
                                      cutoff = 0.5)

  bma_prerec_m2_100_1[[e]] <- prerec(amInferred = ppm_m2_100_1[[e]],
                                     amTrue = am_m2,
                                     cutoff = 0.5)
...
save(bma_m1_100_02,
     ...
     bma_g2_100_05,
     bma_g2_100_1,
     file = '/Users/Evatar/Desktop/bma_ge_N_100.RData')

save(bma_prerec_m1_100_02,
     bma_prerec_m1_100_05,
     bma_prerec_m1_100_1,
     bma_prerec_m2_100_02,
     bma_prerec_m2_100_05,
     bma_prerec_m2_100_1,
     bma_prerec_g2_100_02,
     bma_prerec_g2_100_05,
     bma_prerec_g2_100_1,
     file = '/Users/Evatar/Desktop/bma_prerec_ge_N_100.RData')
